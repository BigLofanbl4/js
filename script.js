...
let arr = [[1, 2, 3], [4, 5], [6]];
arr = arr.reduce((arr, elem) =>  arr.concat(elem), []);
console.log(arr);

function loop(value, condition, body, update) {
...
console.log(loop(1, (val) => val < 10, console.log, (val) => ++val));
...
function dominantDirection(text) {
  ...
  return scripts.reduce((max, elem) => {
    if (max.count < elem.count) {
      max = elem;
    }
    return max;
  }).name;
